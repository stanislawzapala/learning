"""
PROJEKT: Interaktywny Model Populacyjny Hassella
Instrukcja uruchomienia:
1. Zainstaluj wymagane biblioteki komendą: pip install -r requirements.txt
2. Uruchom aplikację komendą: streamlit run app.py
"""
...
def plot_trajectory(xt, l, b, iterations=2000, show_last=100):
    '''
    Rysuje trajektorię populacji w czasie dla podanych parametrów.
    xt: początkowa populacja
    l: tempo wzrostu (lambda)
    b: typ konkurencji (beta)
    iterations: ile iteracji wykonać w sumie (domyślnie 2000)
    ...
    ax_traj.set_xlabel("Pokolenie (t) - ostatnie N iteracji")
    ax_traj.set_ylabel("Populacja (X)")
    ax_traj.grid(True)
    st.pyplot(fig_traj) 

def plot_bifurcation_map(x_start, lambda_vals, beta_val, iterations = 2000, show_last=100):
    '''
    ...
    iterations: ile iteracji wykonać dla każdego lambda (domyślnie 2000)
...
def plot_cobweb(x_0, l, b, iterations=100):
    ...
    for i in range(len(x_history) - 1):
        # Rysujemy linię pionową (x(t) do f(x(t)))
    ...
    ax.plot(px, py, color='red', linewidth=1, alpha=0.7)
    ax.plot(x_0, 0, marker='o', color='green', markersize=6, label=f'Wartość początkowa populacji')
    ax.set_title(f" (λ = {l:.2f}, β = {b:.2f})")
...
st.markdown("""
**Model Hassella** opisuje dynamikę populacji z nienakładającymi się pokoleniami.  
Równanie modelu:
$$X(t+1) = \\lambda \\cdot X(t) \\cdot (1 + X(t))^{-\\beta}$$
""")

st.info(r"""
**Słowniczek parametrów:**
* **$X_0$ (Populacja początkowa):** Przyjmuje wartości z przedziału $(0, 1]$. Reprezentuje początkowe wykorzystanie zasobów (np. $0.1$ oznacza start z poziomu 10% maksymalnej pojemności środowiska).
* **$\lambda$ (Tempo wzrostu):** Kontroluje, jak szybko populacja mnożyłaby się bez żadnych ograniczeń środowiskowych. Im wyższa wartość, tym większy potencjał wzrostu.
* **$\beta$ (Typ konkurencji):** Wpływa na to, jak rywalizacja między osobnikami ogranicza ich wzrost. Im wyższa wartość, tym silniejsza, bardziej drastyczna konkurencja o kurczące się zasoby.
""")
...
# TRAJEKTORIA
st.markdown("### 1. Pojedyncza Trajektoria dla ustalonych parametrów")
st.write("Dostosuj parametry Lambda i Beta, aby zobaczyć trajektorię populacji w czasie. Wyświetlane jest ostatnie N iteracji.")
col1_1, col1_2, col1_3 = st.columns(3)

with col1_1:
    l_val = st.slider("Parametr Lambda - Tempo wzrostu", 0.1, 300.0, 15.0, 0.1)
with col1_2:
    b_val = st.slider("Parametr Beta - Typ konkurencji", 0.1, 20.0, 2.2, 0.1)
with col1_3:
    last_n = st.slider("Pokazywane iteracje", 100, 1000, 100, 100)
...
# MAPA POWROTU
st.markdown("### 2. Mapa wartości X(t) vs X(t+1)")
...
# MAPA BIFURKACJI
st.markdown("### 3. Mapa Bifurkacji")

col2_1, col2_2 = st.columns(2)
with col2_1:
    beta_const = st.slider("Beta dla mapy bifurkacji", 0.1, 20.0, 2.2, 0.1, key="bif_beta")
with col2_2:
    last_n_bif = st.slider("Pokazywane iteracje dla mapy bifurkacji", 100, 1000, 100, 100, key="bif_last_n")
...
# COBWEB
st.markdown("### 4. Diagram Pajęczynowy")
...
# ANIMACJA
st.markdown("### 5. Animacja przejścia od stabilności do chaosu")
st.write("Płynna zmiana parametru Lambda przy ustalonym parametrze Beta, aby zaobserwować moment utraty stabilności.")
...
with col_a2:
    ...
    anim_last_n = st.slider("Pokazywane iteracje (N ostatnich)", 100, 1000, 100, 100, key="anim_last_n")
with col_a3:
